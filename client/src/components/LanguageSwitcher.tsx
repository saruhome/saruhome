...
export function LanguageSwitcher({ elevated = false, embedded = false }: { elevated?: boolean; embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "utility-menu-slot utility-menu-slot-language" : `mobile-language-hud fixed bottom-4 right-4 z-[70] md:bottom-6 md:right-6 ${elevated ? "" : ""}`}>
      {isOpen && (
        <div
          id={embedded ? "utility-language-options" : "language-options"}
          className={`pixel-hud-panel absolute bottom-full mb-2 flex items-center gap-1 border-[var(--player-primary)] bg-[#05080dcc] p-1.5 font-rajdhani text-[0.62rem] font-black uppercase tracking-[0.2em] text-white/75 shadow-[3px_3px_0_rgba(0,0,0,0.55)] ${embedded ? "left-0" : "right-0"}`}
          role="menu"
